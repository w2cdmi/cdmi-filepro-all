...
public final class S3Jets3tProperties extends Jets3tProperties
{
  private static final long serialVersionUID = 1L;
  private static final Map<String, S3Jets3tProperties> PROPERTIES = new ConcurrentHashMap<String, S3Jets3tProperties>(
    1);

  private static final Logger LOGGER = LoggerFactory.getLogger(S3Jets3tProperties.class);

  public static S3Jets3tProperties getInstance(S3EndPointInfo endpoint)
  {
    S3Jets3tProperties jets3tProperties = null;

    String propertiesKey = endpoint.getIP() + '_' + endpoint.getHttpPort() + '_' + 
      endpoint.getHttpsPort();
    if (PROPERTIES.containsKey(propertiesKey))
    {
      jets3tProperties = (S3Jets3tProperties)PROPERTIES.get(propertiesKey);
      return jets3tProperties;
    }

    jets3tProperties = new S3Jets3tProperties();
