...
Page({
  ...
  onLoad: function (options) {
    ...
    ownerId = options.oId;
    nodeId = options.nId;
    nodeId_c = options.nId;
    // /1307/2

    // ownerId = "1307" || options.oId;
    // nodeId = "12" || options.nId;
    // nodeId_c = "12" || options.nId;
      ...
      session.invokeAfterLogin(function () {
        ...
        file.getNodeInfo(ownerId, nodeId, function (info) {
          ...
          if (typeof (info.thumbnailUrl) != 'undefined' && info.thumbnailUrl != "" && utils.isShowThumbnail(info.name)) {
            ...
            file.getFileDownloadUrl(ownerId, nodeId, function (data) {
        ...
        //1307 / 5
        //// 点赞 / 判断当前用户是否对指定文件点赞
        page.ispre({ ownerId: NODEID_OWNERID });
        page.countpraise();
        // 点赞 / 获取某个信息所获得的点赞人员列表
  ...
  onShareAppMessage: function (options) {
    return {
      title: fileName,
      path: '/disk/index?type=file&eId=' + enterpriseId + "&oId=" + ownerId + "&nId=" + nodeId + "&fId=" + getApp().globalData.cloudUserId + "&fName=" + encodeURIComponent(getApp().globalData.userName) + "&eName=" + encodeURIComponent(getApp().globalData.enterpriseName),
      imageUrl: utils.getImgSrcOfShareCard(fileName),
  ...
  onFileItemClick: function (e) {
    ...
    if (ext == 'mp3') {
      file.getFileDownloadUrl(fileInfo.ownedBy, fileInfo.id, (res) => {
        var url = utils.replacePortInDownloadUrl(res.downloadUrl);
        wx.navigateTo({
