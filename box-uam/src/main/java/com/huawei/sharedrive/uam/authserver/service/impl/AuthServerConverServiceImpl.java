...
public class AuthServerConverServiceImpl implements AuthServerConverService
{
    ...
    public LdapDomainConfig convertedToObject(AuthServer authConfig)
    {
        ...
        try
        {
            ...
            {
                document = DocumentHelper.parseText(ldapBasicXml);
            ...
            {
                document = DocumentHelper.parseText(filedMappingXml);
                root = document.getRootElement();
            ...
            {
                document = DocumentHelper.parseText(ldapNodesFilterXml);
                root = document.getRootElement();
        ...
        LdapDomainConfig ldapDomainConfig, String type)
    {
        ...
        if (null != ldapBasicConfig)
        {
            if (type.equals(TRANS_BASIC))
            {
                ...
                selLdapBasicConfig.setIsCheckIp(ldapBasicConfig.getIsCheckIp());
                ...
                selLdapBasicConfig.setSecurityProtocol(ldapBasicConfig.getSecurityProtocol());
            }
