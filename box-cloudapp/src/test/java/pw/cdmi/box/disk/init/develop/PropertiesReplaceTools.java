...
public class PropertiesReplaceTools
{
    ...
    {
        String jdbcUrlValue = prop.getProperty("jdbc.url");
        jdbcUrlValue = jdbcUrlValue.replaceAll("<sysdb_ip>", LocalConfig.getMySQLIp());
        prop.setProperty("jdbc.url", jdbcUrlValue);
        
        prop.setProperty("zookeeper.server", LocalConfig.getZkIp());
       
        prop.setProperty("cache.uam.server.ips", LocalConfig.getMemcachedIp());
        ...
        prop.setProperty("uam.client.valid.server.cert", "false");
