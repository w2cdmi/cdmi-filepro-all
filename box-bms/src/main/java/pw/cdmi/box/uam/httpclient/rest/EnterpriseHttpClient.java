...
public class EnterpriseHttpClient
{
    ...
    public RestEnterpriseAccountResponse getCurrentAccountInfo(AuthApp authApp, EnterpriseAccount account)
    {
        Map<String, String> headers = getAppAuthHeader(authApp.getUfmAccessKeyId(), authApp.getUfmSecretKey());
        TextResponse response;
        try
        {
            response = ufmClientService.performGetText(ACCOUNT_URL + "/" + account.getAccountId(), headers);
