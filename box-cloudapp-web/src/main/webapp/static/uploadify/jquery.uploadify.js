...
/* upload bug in ie9/10, can not select files */
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm(existMsg1 + file.name + existMsg2);
					if (!replaceQueueItem) {
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel">\
						<a onclick="$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>\
					</div>\
				...
				$
						.ajax({
							...
							success : function(data) {
								if (data == 1) {
									var overwrite = confirm(existServerMsg1 + file.name
											+ existServerMsg2);
									if (!overwrite) {
										this.cancelUpload(file.id);
										$('#' + file.id).remove();
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				$('#' + file.id).find('.data').html(' - ' + completeMsg);
			}
