        ...
        if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {//这里采用同步方式打开文档。我要返回值
            if (httpclient.Send()) {
                ...
                if (this.hiddenSaveLocal(httpclient, this, false, false)) {//下载成功时
					...
					if(this.OpenLocalFile(this.TmpFile)==0){ //打开本地磁盘文件
                        this.getOfficeVersion();//打开文档后，判断当前office版本
                        return true;
                    }else{
...
/*保存到本地  isHidden 是否隐藏文件；isInsertFile是否是插入文件*/
    this.hiddenSaveLocal = function (httpclient, webOffice, isHidden, isInsertFile, OtherName) {
        ...
        if (httpclient.Status == 200) {
            ...
            if (OtherName == undefined || OtherName == "") {
            	  //重新命名一个文档，防止控件多开的情况，导致前一个控件出现异常情况s
            		var tempFileName = Math.round(Math.random() * 100000000) + "." + this.FileType;
                OtherName = tempFileName;//webOffice.FileName;
            }
            this.TmpFile = filePath + tempName + OtherName;
            httpclient.ResponseSaveToFile(this.TmpFile); //临时文件
        		httpclient.Clear();
            return true;
        }
        httpclient.Clear();
