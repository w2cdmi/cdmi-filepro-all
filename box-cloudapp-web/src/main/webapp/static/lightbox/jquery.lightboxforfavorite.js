...
(function($) {
	$.fn.lightboxFVR = function(options) {
		var rotateNum = 0;
		// build main options
		var opts = $.extend({}, $.fn.lightboxFVR.defaults, options);
		...
		*/
		function showLightbox(imageObject) {
            ...
            if ((!getImageSetOf(imageObject) || (getImageSetOf(imageObject) == '')) && !opts.allSet) {
				...
				opts.imageArray.push(new Array(data[1], opts.displayTitle ? data[2] : '', data[3], data[4]));
			} else {
				// if image is part of a set..
				$("a").each(function() {
                    if(this.rel && (getImageSetOf(this) == getImageSetOf(imageObject))) {
						opts.imageArray.push(new Array((this.rel).split("|/|")[1], opts.displayTitle ? (this.rel).split("|/|")[2] : '', (this.rel).split("|/|")[3], (this.rel).split("|/|")[4]));
					}
				});
			}
		...
		function doChangeImage(flag) {
			...
			var _urlGroup = $.fn.lightboxFVR.getPreviewUrl( opts.imageArray[opts.activeImage][0] );
		...
		function preloadNeighborImages() {
			...
			if (opts.loopImages && opts.imageArray.length > 1) {
				...
				preloadNextImage.src = ($.fn.lightboxFVR.getPreviewUrl( opts.imageArray[(opts.activeImage == (opts.imageArray.length - 1)) ? 0 : opts.activeImage + 1][0] )).split("|||")[0];

				preloadPrevImage = new Image();
				preloadPrevImage.src = ($.fn.lightboxFVR.getPreviewUrl( opts.imageArray[(opts.activeImage == 0) ? (opts.imageArray.length - 1) : opts.activeImage - 1][0] )).split("|||")[0];
			} else {
				if ((opts.imageArray.length - 1) > opts.activeImage) {
					preloadNextImage = new Image();
					preloadNextImage.src = ($.fn.lightboxFVR.getPreviewUrl( opts.imageArray[opts.activeImage + 1][0] )).split("|||")[0];
				}
				if (opts.activeImage > 0) {
					preloadPrevImage = new Image();
					preloadPrevImage.src = ($.fn.lightboxFVR.getPreviewUrl( opts.imageArray[opts.activeImage - 1][0] )).split("|||")[0];
				}
			}
		};
		...
		*/
		...
		function updateNav() {
			...
			$('#btnDownload').click(function(){
				$.fn.lightboxFVR.downloadImg( opts.imageArray[opts.activeImage][2], opts.imageArray[opts.activeImage][3] );
				return false;
			})
		};
	...
	/* close lightbox */
	$.fn.lightboxFVR.close = function(){
		$(document).unbind('keydown');
		$('#lightbox, #overlay').remove();
	}

	$.fn.lightboxFVR.defaults = {
		allSet: false,
		overlayOpacity: 0.6,
		borderSize: 10,
