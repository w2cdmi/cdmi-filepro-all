...
(function() {
	...
	(function() {
		...
		var init = function() {
			...
			if(curCfg.iframe){
				//调用公共组件spinLoading
				ymPromptSpiner = new Spinner(optsMiddleSpinner).spin(document.getElementById('ymPromptLoadDiv'));
				//判断加载是否完成
				var oFrm = ym_body.firstChild;
				oFrm.onload = oFrm.onreadystatechange = function() {
				    if (this.readyState && this.readyState != 'complete') return;
				    else{
				    	//调用common.js的公共方法卸载加载动画
				        comboxRemoveLoading("ymPromptLoadDiv");
				    }
				}
			}
		...
		var destroy = function() { ! curCfg.useSlide && maskVisible(false);
			winVisible(false);
			curCfg.iframe ? ymPromptSpiner.stop() : '';
		}; //取得iframe
