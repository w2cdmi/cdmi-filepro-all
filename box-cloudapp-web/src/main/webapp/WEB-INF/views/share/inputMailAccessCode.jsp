            ...
            <div class="form-con">
                <form class=" form-horizontal" id="mailForm" name="mailForm" action="sendAccessCode" method="post">
                    ...
                    <div class="control-group">
                        <label class="control-label" for=""><em>*</em><spring:message code="common.field.email"/>:
                        </label>
                        <div class="controls">
                            ...
                            <span class="help-block"><spring:message code="link.info.mailForAccessCode"/></span>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            error: function (request) {
                if (request.status == 405) {// jump login
                    ...
                    $("#mail").val("").focus();
                    handlePrompt("error", "<spring:message code='link.error.mailAuthFail' />");
                }
            },
            success: function (data) {
    ...
    $.validator.addMethod(
        ...
        function (value, element) {
            ...
            if (!pattern.test(value) || value.length < 5 || value.length > 255) {
                return false;
            }
            return true;
        },
        $.validator.format("<spring:message code='link.set.invalidEmail'/>")
    );
