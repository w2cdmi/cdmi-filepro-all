    ...
    <div class="body-con clearfix">
        <div class="pull-left clearfix">
            ...
            <div class="files-block clearfix">
                <div id="fileListNull" class="share-null"><spring:message code="shareLinks.MySharedEmpty"/></div>
                <div id="fileList"></div>
...
<script type="text/javascript">
    ...
    var opts_page = null;
    var viewMode = "file";
    var listViewType = getRootCookie("teamlinkListViewType");
        ...
        $.fn.comboGrid.selectOp = function (allSelectData) {
            $.fn.comboGrid.initGridMenu = function () {
                ...
                if (allSelectData.length == 0) {
                    ...
                    var nodePermission = getNodePermission(item.ownedBy, item.id);
                    if (nodePermission["publishLink"] == 1) {
                        menuData = {"m_cancelLink": M.m_cancelLink, "m_viewLink": M.m_viewLink};
                    }
                } else {
                    var permissionFlag = getNodePermission(teamId, 0);
                    if (permissionFlag["publishLink"] == 1) {
    ...
    function viewType(type) {
        ...
        listViewType = type;
        setRootCookie("teamlinkListViewType", listViewType);
    }
        ...
        $.ajax({
            ...
            error: function (request) {
                if (request.status == 403) {
                    ymPrompt.errorInfo({
                        title: "<spring:message code='share.menu.shareLinks'/>",
                        message: "<spring:message code='teamspace.link.operation.authority'/>",
                        handler: function () {
                            window.location = "${ctx}/teamspace/";
                            return;
                        }
                    });
                    return;
    ...
    function shareLink(ownedByTmp, nodeIdTmp) {
        ...
        if (nodePermission["authorize"] != 1) {
            ymPrompt.errorInfo({
                title: "<spring:message code='shareLinks.viewLinks'/>",
                message: "<spring:message code='teamspace.link.operation.authority'/>",
                handler: function () {
                    window.location = "${ctx}/teamspace/";
                    return;
    ...
    function cancelLink() {
        ...
        for (var i = 0; i < objData.length; i++) {
            var nodePermission = getNodePermission(teamId, objData[i].id);
            if (nodePermission["authorize"] != 1) {
                ymPrompt.errorInfo({
    ...
    function getNodePermission(ownedByTmp, parentId) {
        var permission = null;
