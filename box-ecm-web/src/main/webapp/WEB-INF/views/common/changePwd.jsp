		...
		<div class="revise-pwd-content">
			<div class="revise-form-con">
				...
				<form class="revise-form" id="modifyPwdForm">
					...
					<p class="set-email-title"><spring:message code="common.mail.set"/></p>
					<div class="revise-group">
						<div class="revise-word-sty">
							<img src="${ctx}/static/skins/default/img/revisepwd_02.png" alt="" />
							<c:if test="${not empty cse:htmlEscape(email)}">
            					<input class="span4" type="text" id="email" name="email" value="${cse:htmlEscape(email)}" readOnly="readonly"/>
            				</c:if>
            				<c:if test="${empty cse:htmlEscape(email)}">
                				<input class="span4" type="text" id="email" name="email" placeholder="请输入您的邮箱" />
            				</c:if>
							<span class="validate-con"><div></div></span>
						</div>
